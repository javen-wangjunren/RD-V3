...
$(document).ready(function () {
  // 首页section2轮播
  var MachineSlicker = ".manufacturing-content-slicker"; // 首页section2轮播数量
  ...
  function calcSlickPos() {
    ...
    // }
    if ($(window).width() > 1400 && MachineSlickerLength > 4 && $(window).width() < 1930) {
      var slickerLeft = $(".rapiddirect-logo")[0].getBoundingClientRect().left || 0;
      $(MachineSlicker).css("left", slickerLeft - 15);
    } else {
      $(MachineSlicker).css({
        left: 0
  ...
  $(".p02-x-slicker .content-show-slicker").slick({
    ...
    // arrows: false,
    infinite: false,
    prevArrow: '<span class="arrow-btn-2 arrow-prev"><i class="fas fa-caret-left"></i></span>',
    nextArrow: '<span class="arrow-btn-2 arrow-next"><i class="fas fa-caret-right"></i></span>'
  }); // p04-3-tab-table
  ...
  $(".p04-3-tab-table .tab-btn-item").on("click", function () {
    ...
    var index = that.index();
    that.addClass("active").siblings().removeClass("active");
    that.parents(".tab-btn-wrap").next().children().eq(index).addClass("active").siblings().removeClass("active");
  }); //cases-categories
  ...
  $(".p05-3-tab .tab-btn-item .title-wrap").on("click", function () {
    ...
    if (that.attr("class").indexOf("active") !== -1) {
      that.parent().children().removeClass("active").find(".main-content").slideUp();
    } else {
      that.parent().children().removeClass("active").find(".main-content").slideUp();
      that.addClass("active");
    ...
    var index = that.index();
    that.parent().prev().children().eq(index).addClass("active").siblings().removeClass("active");
  }); // vr-slicker
  ...
  if ($(".p04-2-table.no-scroll").length > 0 && $(window).width() > 1024) {
    ...
    var positionY = $("#wpadminbar").length ? navHeight + $("#wpadminbar").height() : navHeight;
  ...
  $(".p02-x-tab .item").click(function () {
    ...
    var index = that.index();
    that.addClass("active").siblings().removeClass("active");
    that.parent().prev().children("img").eq(index).addClass("active").siblings().removeClass("active");
  });

  if ($(".pp-info-box-carousel-wrap").length > 0) {
    $(".pp-info-box-carousel-wrap").on("click", ".pp-info-box-icon-wrap", function () {
      var that = $(this);
  ...
  if ($(".elementor-image-carousel-wrapper").length > 0) {
    $(".elementor-image-carousel-wrapper").on("click", ".swiper-slide", function () {
      console.log(111);
      var that = $(this);
      var src = that.find("img").attr("src");
      $("body").append("\n                <section id=\"img-popup\">\n                    <span class=\"close\"><i class=\"fas fa-times\"></i></span>\n                    <div class=\"container\">\n                        <img src=\"".concat(src, "\">\n                    </div>\n                </section>\n            "));
  ...
  if ($(window).width() < 768) {
    $(".pp-image-accordion .pp-image-accordion-item").eq(0).css("flex", "3").addClass("active");
    $(".pp-image-accordion .pp-image-accordion-item").on("mouseenter", function () {
      var that = $(this);
