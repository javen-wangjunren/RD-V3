...
/**
 * Elementor mml_breadcrumbs widget.
 *
 * Elementor widget that displays an eye-catching headlines.
 *
 * @since 1.0.0
 */
class Widget_mml_breadcrumbs extends Widget_Base {

    /**
     * Get widget name.
     *
     * Retrieve mml_breadcrumbs widget name.
     *
     * @since 1.0.0
     * @access public
     *
    ...
    public function get_name() {
        return 'MML Breadcrumbs';
    }

    /**
     * Get widget title.
     *
     * Retrieve mml_breadcrumbs widget title.
     *
     * @since 1.0.0
     * @access public
     *
    ...
    public function get_title() {
        return __( 'MML Breadcrumbs', 'elementor' );
    }

    /**
     * Get widget icon.
     *
     * Retrieve mml_breadcrumbs widget icon.
     *
     * @since 1.0.0
     * @access public
     *
    ...
    public function get_icon() {
        return 'ppicon-breadcrumbs mml-icon';
//        return 'ppicon-breadcrumbs power-pack-admin-icon';
     ...
     *
     * Retrieve the list of categories the mml_breadcrumbs widget belongs to.
     *
    ...
    public function get_keywords() {
        return [ 'mml_breadcrumbs', 'title', 'text' ];
    }

    /**
     * Register mml_breadcrumbs widget controls.
     *
...
//                ),
        ...
        // 获取所有的分类
        $cates = get_taxonomies([],'objects');
        $cates_arr = [];
        if ($cates){
            foreach ($cates as $key => $cate) {
//                var_dump($key);
//                var_dump($cate);die;
                $cates_arr[$key] = $cate->label.'  ('.$key.')';
            }
        }

        $this->add_control(
            'show_contents_page_link',
            array(
                'label'        => __( 'Show Contents Page Link', 'powerpack' ),
                'type'         => Controls_Manager::SWITCHER,
                'default'      => 'yes',
                'label_on'     => __( 'On', 'powerpack' ),
                'label_off'    => __( 'Off', 'powerpack' ),
        ...
        $this->add_control(
            'contents_page_link',
            array(
                'label'     => 'Contents Page Link',
                'type'      => Controls_Manager::SELECT,
                'options'=>$pages_arr,
                'condition'    => array(
                    'show_contents_page_link' => 'yes',
                ),
            )
        );
        ...
        $this->add_control(
            'show_categorys_page_link',
            array(
                'label'        => __( 'Show Categorys Page Link', 'powerpack' ),
                'type'         => Controls_Manager::SWITCHER,
                'default'      => 'yes',
                'label_on'     => __( 'On', 'powerpack' ),
                'label_off'    => __( 'Off', 'powerpack' ),
        ...
        $this->add_control(
            ...
            array(
                'label'        => __( 'Show Parent Category', 'powerpack' ),
                'type'         => Controls_Manager::SWITCHER,
                'default'      => 'yes',
                'label_on'     => __( 'On', 'powerpack' ),
                'label_off'    => __( 'Off', 'powerpack' ),
...
//                ),
    ...
    /**
     * Render mml_breadcrumbs widget output on the frontend.
     *
...
//        $settings = $this->get_settings_for_display();
    ...
    protected function render_breadcrumbs( $query = false ) {
        ...
        if ('yes' === $settings['show_categorys_page_link'] && $settings['select_category']){
            $custom_taxonomy = $settings['select_category'];
        }else{
                    ...
                    $this->add_render_attribute(
                        array(
                            ...
                            'breadcrumbs-item-cpt-crumb' => array(
                                ...
                                'href'  => $settings['contents_page_link']?get_permalink($settings['contents_page_link']):$post_type_archive,
                                'title' => $post_type_object->labels->name,
//                                'title'=>123
                            ),
                        )
                    );
                    if ('yes' === $settings['show_contents_page_link'] && $settings['contents_page_link']) {
                        echo '<li ' . $this->get_render_attribute_string('breadcrumbs-item-cpt') . '><a ' . $this->get_render_attribute_string('breadcrumbs-item-cpt-crumb') . '>' . get_the_title($settings['contents_page_link']) . '</a></li>';
                        $this->render_separator();
                    }
                ...
                if ( empty( $last_category ) && ! empty( $custom_taxonomy ) && $taxonomy_exists ) {
                    ...
                    if ($link){
                        ...
                        array_pop($category);
                        $category_slug = array_pop($category);
//    var_dump($category_slug);
                        $taxonomy_terms[0] = get_term_by('slug',$category_slug,$custom_taxonomy);
                    }
                ...
                if ( ! empty( $last_category ) ) {
                    echo $cat_display;
//                    echo '<li class="pp-breadcrumbs-item pp-breadcrumbs-item-cat"><a class="pp-breadcrumbs-crumb pp-breadcrumbs-crumb-link pp-breadcrumbs-crumb-cat" href="' . get_term_link( $last_category ) . '">' . $last_category->name . '</a></li>';
//                    $this->render_separator();
                ...
                elseif ( !empty($taxonomy_terms[0])  ) {
                    foreach ( $taxonomy_terms as $index => $taxonomy ) {
                        ...
                        //判断是否输出多个分类层级
                        if ('yes' === $settings['show_parent_category']){
                            $cates[] = $taxonomy;
                            while ($taxonomy->parent !== 0){
                                $taxonomy = get_term_by('id', $taxonomy->parent, $custom_taxonomy );
                                $cates[] = $taxonomy;
                            }

                            while ($cate = array_pop($cates)){
                                $cat_id = $cate->term_id;
                                $cat_nicename = $cate->slug;
                                $cat_link = get_term_link($cate->term_id, $custom_taxonomy);
                                $cat_name = $cate->name;
            ...
            elseif ( $query->is_page() ) {
                ...
                // Standard page
                if ('yes' === $settings['show_contents_page_link'] && $settings['contents_page_link']) {
                    echo '<li class="pp-breadcrumbs-item"><a class="pp-breadcrumbs-crumb pp-breadcrumbs-crumb-link " href="' . get_permalink( $settings['contents_page_link'] ) . '" title="' . get_the_title( $settings['contents_page_link'] ) . '">' . get_the_title( $settings['contents_page_link'] ) . '</a></li>';
                    $this->render_separator();
                }
                if ( $post->post_parent ) {
