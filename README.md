...
## npm start
 需要编写 es6 代码的同学需要先在 src/js 目录下创建 js 文件，然后启动 npm start 命令。es6 源码会编译生成到 dist/js 文件夹下。 我们只需要在页面中引用 dits/js 文件下的 js 即可。 npm start 是一个复合命令，执行它之后，它会同时启动 dev 和 dev2命令， dev 命令是用来编译 scss 文件的， dev2 是用来编译 es6 代码的。
...
## npm run replace
在执行这个命令前，请先执行 npm run createImg 命令。并且要在后端套完数据后才执行此命令。
1、将 img-slice.csv 文件里面的 alt 替换到 php 模版文件中。
2、将 home、footer、header 模版的图片增加懒加载的功能
ps: 此命令执行一次即可，不需要多次执行

## npm run wpconfig

执行 npm run wpconfig dbname（数据库名）自动生成 wp-config.php 文件

## 中台项目导入alt
脚本放在共享文件夹192.168.0.254\anton2.0\Project\alt-csv\fill_alt.php，将此文件放置在所用主题的templates目录下，脚本注释中有使用说明
